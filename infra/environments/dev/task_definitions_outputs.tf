output "ecs_task_definition_arns" {
  description = "Active ECS task definition ARNs indexed by application service."

  value = {
    for service_name, task_definition
    in data.aws_ecs_task_definition.active_service :
    service_name => task_definition.arn
  }
}

output "ecs_task_definition_families" {
  description = "Active ECS task definition families indexed by application service."
...
output "ecs_task_definition_revisions" {
  description = "Active ECS task definition revisions indexed by application service."
